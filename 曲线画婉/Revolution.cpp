...
void CRevolution::ReadNurbsControlPoint(CP2*ctrlP2,double*vW,int n,int q)
{
	RelaseMemory();
	m=8,p=2;
	this->n=n,this->q=q;
...
void CRevolution::CalculateControlGrid(CP2*pt)
{
	for(int j=0;j<n+1;j++)
	{
		...
		i=1;
		Vertex[(m+1)*j+i]=CP3(pt[j].x,pt[j].y+(i*5),r);
		i=2;
		Vertex[(m+1)*j+i]=CP3(0,pt[j].y+(i*5),r);
		i=3;
		Vertex[(m+1)*j+i]=CP3(-pt[j].x,pt[j].y+(i*5),r);
		i=4;
		Vertex[(m+1)*j+i]=CP3(-pt[j].x,pt[j].y+(i*5),0);
...
void CRevolution::DrawRevolution(CDC *pDC)
{
	CP3**P=new CP3*[n+1];
	for(int j=0;j<n+1;j++)
		P[j]=new CP3[m+1];
